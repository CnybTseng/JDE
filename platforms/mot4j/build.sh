...
g++ mot4j.cpp -fPIC -shared -o libmot4j.so \
    ...
    -I/usr/local/opencv-4.1.1/include/opencv4 \
    -I/usr/local/jsoncpp/include \
    -Lmot/lib \
    -L/usr/local/lib64 \
    -L/usr/local/jsoncpp/lib \
    -L/usr/local/opencv-4.1.1/lib \
    -lmot \
    -ljson \
...
echo step 3: make distribution package ...
rsync -a ./com ./release/
rsync -a ./*.so ./release/lib
rsync -a ./mot/lib/*.so ./release/lib
rm -rf ./com
rm -f ./*.so

echo step 4: make test.class ...
cd release
javac test.java
...
# export LD_LIBRARY_PATH=/usr/local/opencv-4.1.1/lib:$LD_LIBRARY_PATH
