...
static void do_work_for_video(int argc, char* argv[], const char* path)
{
    std::thread::id tid = std::this_thread::get_id();
    ...
    for (int test = 0; test < iters; ++test)
    {
        ...
        if (!cap.isOpened())
        {
            fprintf(stderr, "VideoCapture(%s) fail\n", path);
            break;
        }
        ...
        {            
            // 读取图像文件和解码成BGR888
            cv::Mat bgr;
            if (cnt > maxframes || !cap.read(bgr))
            {
                fprintf(stdout, "VideoCapture.read() fail!\n");
                break;
            }
#if (!PROFILE)
            auto start = std::chrono::high_resolution_clock::now();
            ...
            auto end = std::chrono::high_resolution_clock::now();
            latency = std::chrono::duration<float, std::milli>(end - start).count();
            fprintf(stdout, "\r%s: %fms", ss.str().c_str(), latency);
#else
            fprintf(stdout, "\r%s: %s", ss.str().c_str());
#endif
            fflush(stdout);
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 3)
    {
        fprintf(stderr, "Usage:\n%s cfg_path images_path [num_thread,[save<0,1>], iters, maxframes]\n", argv[0]);
        return -1;
    }
